...
# An R function to generate a transition matrix
...
# variable, and the second column is the "to" variable.  Here, column 1 is 1997 canopy height
# and column 2 is 2006 canopy height
# max.height is the maximum height for which transitions will be calculated.  In this case, it is 56 m
# which is taller than the tallest canopy height measurement in 1997 and 2006
...
# An R function to create a transition probability matrix from a transition matrix
...
# An R function to simulate the stable stage distribution using non-parametric bootstrap sampling
...
#### Read in data ####
  ...
  setwd("~/Desktop/Chapter3/LaSelvaBlowdown")

#Read in the LiDAR data from an ENVI file

  load("CHMs_AOI_res500.RData")
  

#If canopy height is < 0 set equal to 0
...
#### Make height transition matrix and transition probability matrix ####
...
#### Simulate steady state from a Dirichlet distribution ####

# Function rdiric
# An R function to simulate observations from a Dirichlet distribution
...
##### Make Figure 3 and calculate other results stats #####
par(mfrow=c(1,1), mar=c(3,3,0,0), oma=c(2,2,2,2))
  ...
  # Find the predicted mean canopy height for steady-state equilibrium
    ...
    mean(predMeanHt)
    quantile(predMeanHt,probs = c(0.25,0.975))
  ...
  # Calculate skewness and kurtosis of projected equilibirum 
    ...
    mean(momentsSS$skew)
    quantile(momentsSS$skew,probs = c(0.25,0.975))
    
    mean(momentsSS$kurtosis)
    quantile(momentsSS$kurtosis,probs = c(0.25,0.975))
    
#### Make height transition matrix and transition probability matrix _ OLD GROWTH ####
...
#### Simulate steady state from a Dirichlet distribution _ OLD GROWTH ####
...
#### Make height transition matrix and transition probability matrix _ SECONDARY ####
...
#### Simulate steady state from a Dirichlet distribution _ SECONDARY ####
